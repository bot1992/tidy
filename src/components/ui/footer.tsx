...
import { SocialMediaLabelType } from "@/src/types"

type Props = FooterType & { socialMedia: SocialMediaType }

const Footer: FC<Props> = function ({ items, text, socialMedia }) {
  return (
    <footer>
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid sm:grid-cols-12 gap-8 py-8 md:py-12">
          ...
          <div className="sm:col-span-12 lg:col-span-4 lg:max-w-xs">
            ...
            <div className="text-lg font-bold text-slate-800">{text}</div>
          </div>

          {items.map(({ id, title, links }) => (
            <div key={id} className="sm:col-span-6 md:col-span-3 lg:col-span-2">
        ...
        <div className="md:flex md:items-center md:justify-between py-6 md:py-8 border-t border-slate-200">
          ...
          <ul className="flex space-x-6 mb-4 md:order-1 md:ml-4 md:mb-0">
            {(socialMedia?.items ?? []).map(({ id, label, link }, i) => (
              <li
                key={id}
                className={classNames({
                  "ml-4": i !== 0,
              ...
              >
                ...
                >
                  <SocialMediaIcon
                    type={(label ?? "").toLowerCase() as SocialMediaLabelType}
